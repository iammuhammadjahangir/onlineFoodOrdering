const assignRole = require("../models/userRoleTasksModel");
const taskModel = require("../models/userTaskModel");
...
exports.getOnlyAllowedTasks = async (req, res, next) => {
  try {
    const { role } = req.params;
    console.log(role);
    const taskId = await assignRole.find({ role: req.params.role , status:true }).populate("task");
    if (!taskId) {
...
const taskNamesArray = taskId.map((taskObject) => taskObject.task.taskName);
    if (!taskNamesArray) {
