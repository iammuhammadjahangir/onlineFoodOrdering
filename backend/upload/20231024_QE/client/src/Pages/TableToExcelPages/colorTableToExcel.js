...
const requiredFields = ["_id", "colorName", "colorDescription"];
const ColorTableToExcel = () => {
  ...
  const uploadData = async () => {
    ...
    const jokesResponse = await getColorr();
    ...
    if (updateProductsData?.length > 0) {
      ...
      const result = await updateColorsTableUsingExcelData(updateProductsData);
      if (result) {
        alert(
    ...
    if (productsWithoutId?.length > 0) {
      const result = await postNewColorsTableUsingExcel(productsWithoutId);
      if (result) {
    ...
    if (postProductsWith_id?.length > 0) {
      ...
      const result = await postNewColorsTableUsingExcel(postProductsWith_id);
      if (result) {
        alert(
...
export default ColorTableToExcel;
